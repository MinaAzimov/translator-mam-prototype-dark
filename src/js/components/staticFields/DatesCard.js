...
constructor (props) {
  ...
  handleChange(date) {
   ...
   handleChangeSecond(date1) {
    this.setState({
      startDateSecond: date1,
    });
  }
	      ...
	      <Card title="Dates" id="dates">
			<CardSection>
					...
					<div>
						<FieldWidgets.Text
						label="Upload Date"
						value={moment.unix(this.props.target.lastModified).format("MM/DD/HH")}
						/>
						<FieldWidgets.Text
						label="Date Created"
						value={moment.unix(this.props.target.lastModified).format("MM/DD/HH")}
				        ...
				        isClearable={true}
  						placeholderText="Please select new date"
  						className="our-custom"
					    />
					   

				        <label> Shoot Date</label>
						<DatePicker
				        selected={this.state.startDateSecond}
...
export default connect(mapStateToProps)(DatesCard);